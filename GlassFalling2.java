   ...
   // X is the current floor you're at. 
    for(int x = 1; x <= floors; x++){
		// value = Math.max(glassFallingRecur(x - 1, sheets - 1), 
			...
			int value = 1 + Math.max(glassFallingRecur(x-1, sheets-1), glassFallingRecur(floors-x,sheets));
			if(value < minTrails){
				minTrails = value;
			}
...
for(int currentFloor = 1; currentFloor <= floors; currentFloor++) {
			...
			for(int currentNumSheet = 2; currentNumSheet <= sheets; currentNumSheet++) {
				...
				for( int x = 2; x <= currentFloor; x++ ) {
					 ...
					 int results = 1+ Math.max(memo[x - 1][ currentNumSheet - 1], memo[currentFloor - x][ currentNumSheet]);
				
					
					if(results < memo[currentFloor][currentNumSheet]) {
						memo[currentFloor][currentNumSheet] = results;
					}
				}
			}
     ...
     int[][] glassFall = new int[sheets + 1][];
        ...
        for (i = 2; i <= sheets; i++) {
            for (j = 2; j <= floors; j++) {
                ...
                for (x = 1; x <= j; x++) {
                    res = 1 + Math.max(glassFall[i - 1][x - 1], glassFall[i][j - x]);
  ...
  public static void main(String args[]){
      ...
      int minTrials2Recur = gf.glassFallingRecur(10, 3);
      int minTrials2Bottom = gf.glassFallingBottomUp(10, 3);
